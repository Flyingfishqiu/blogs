    ...
    $.get('/index/bogs_list', params, function (response) {
        ...
        if (response.errno == '0') {
            ...
            for (var i = 0; i < response.data.blogs_list_dict.length; i++) {
                ...
                content2 += '<a href="#" class="pic"> <img class="lazyload" data-original="temp/art.jpg" src="'+blogs.index_image_url+'" alt="应该选"/></a>'
                ...
                content2 += '</div>'
                content2 += '<div class="desc">'+blogs.digest+'</div>'
                content2 += ' </li>'
                // append表示将新的数据追加到旧的数据的后面；html表示将新的数据替换到旧的数据的后面；
                $(".index_arc").append(content2)
            }
        } else {
